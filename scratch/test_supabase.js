...
async function test() {
  ...
  if (adminError) {
    ...
    if (adminData.length > 0) {
      ...
      console.log('Sample employee photo:', adminData[0].photo);
  ...
  console.log('\n--- 4. Checking RLS policies on tables ---');
  const { data: policies, error: polErr } = await adminClient
    .rpc('get_policies')
    .catch(() => ({ data: null, error: 'no rpc' }));
  
  // Query pg_policies directly via admin query if possible, or sql query
  const { data: polData, error: polDataErr } = await adminClient
    .from('pg_policies')
    .select('*');
  console.log('pg_policies access:', polDataErr ? polDataErr.message : (polData ? polData.length : 'none'));

  console.log('\n--- 5. Checking buckets with adminClient ---');
  ...
  console.log('\n--- 6. Checking sample time_records with adminClient and anonClient ---');
  ...
  if (trAdmin && trAdmin.length > 0) {
    ...
    console.log('Sample time_record:', trAdmin[0]);
  }

  console.log('\n--- 7. Checking sample evaluations ---');
