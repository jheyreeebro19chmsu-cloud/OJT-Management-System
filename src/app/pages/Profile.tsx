...
export function Profile() {
  ...
  return (
    <div className="space-y-4">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-blue-300 text-xs mt-0.5">{employee.position} • {employee.department}</p>
            {employee.faceRegistered ? (
        ...
        <div className="mt-4">
          <div className="flex justify-between text-xs mb-1.5">
            ...
            <span className="text-white font-medium">{totalHours.toFixed(1)} / {employee.requiredHours} hrs ({Math.round(progressPct)}%)</span>
          </div>
      ...
      {evaluation && evaluation.status === 'final' && (() => {
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="ml-auto text-xs text-gray-500">
                {new Date(evaluation.evaluatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
              </span>
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">{hostFeedback.hostCompany} - {hostFeedback.hostName}</p>
            </div>
            <span className="ml-auto text-xs text-gray-400">
              {new Date(hostFeedback.submittedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            </span>
          </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2 text-xs mb-4">
            <div className="bg-gray-50 rounded-xl p-2 text-center">Attendance: <span className="font-semibold">{hostFeedback.attendanceScore}%</span></div>
            <div className="bg-gray-50 rounded-xl p-2 text-center">Performance: <span className="font-semibold">{hostFeedback.performanceScore}%</span></div>
            <div className="bg-gray-50 rounded-xl p-2 text-center">Attitude: <span className="font-semibold">{hostFeedback.attitudeScore}%</span></div>
            <div className="bg-gray-50 rounded-xl p-2 text-center">Communication: <span className="font-semibold">{hostFeedback.communicationScore}%</span></div>
            <div className="bg-gray-50 rounded-xl p-2 text-center">Teamwork: <span className="font-semibold">{hostFeedback.teamworkScore}%</span></div>
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {!editing ? (
            <button onClick={() => setEditing(true)} className="text-xs text-blue-600 hover:text-blue-800 flex items-center gap-1">
              <Edit2 size={12} /> Edit
            </button>
          ) : (
            <div className="flex gap-2">
              <button onClick={handleSave} className="text-xs text-green-600 hover:text-green-800 flex items-center gap-1">
                <Check size={12} /> Save
              </button>
              <button onClick={() => setEditing(false)} className="text-xs text-red-500 hover:text-red-700 flex items-center gap-1">
                <X size={12} /> Cancel
              </button>
            </div>
          )}
        ...
        {editing ? (
          <div className="space-y-3">
            ...
            <div>
              ...
              <input value={form.supervisorName} onChange={e => setForm(p => ({ ...p, supervisorName: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
        ) : (
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        <Section title="Company Information" icon={<Building size={15} className="text-blue-700" />}>
          ...
          <InfoRow label="Start Date" value={new Date(employee.startDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} />
          <InfoRow label="End Date" value={new Date(employee.endDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} />
        </Section>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
        <Section title="Schedule Info" icon={<Clock size={15} className="text-blue-700" />}>
          <InfoRow label="OJT Start" value={new Date(employee.startDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} />
          <InfoRow label="OJT End" value={new Date(employee.endDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} />
          <InfoRow label="Hours Left" value={`${Math.max(0, employee.requiredHours - totalHours).toFixed(1)} hrs remaining`} />
        </Section>
      </motion.div>
    </div>
  );
