...
const TYPE_CONFIG: Record<Announcement['type'], { label: string; color: string; bg: string; border: string; icon: React.ReactNode }> = {
  info:    { label: 'Information', color: 'text-blue-700',  bg: 'bg-blue-50',  border: 'border-blue-200',  icon: <Info size={14} /> },
  warning: { label: 'Warning',     color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200', icon: <AlertTriangle size={14} /> },
  success: { label: 'Good News',   color: 'text-green-700', bg: 'bg-green-50', border: 'border-green-200', icon: <CheckCircle size={14} /> },
  urgent:  { label: 'Urgent',      color: 'text-red-700',   bg: 'bg-red-50',   border: 'border-red-300',   icon: <Bell size={14} /> },
};
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{announcements.length} total • {announcements.filter(a => !isExpired(a)).length} active</p>
        </div>
        <button
          onClick={openAdd}
        ...
        <div className="space-y-3">
          {sorted.map((ann, idx) => {
            ...
            return (
              ...
              >
                <div className={`h-1 ${ann.type === 'info' ? 'bg-blue-500' : ann.type === 'warning' ? 'bg-amber-500' : ann.type === 'success' ? 'bg-green-500' : 'bg-red-500'}`} />
                <div className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {expired && <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">Expired</span>}
                      </div>
                      <p className="text-sm text-gray-600 mt-1.5 leading-relaxed">{ann.content}</p>
                      {ann.photo && (
                        <img src={ann.photo} alt="" className="mt-2 rounded-xl border border-gray-200 max-h-48 object-cover" />
                      )}
                      {ann.reminder && (
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-gray-400">
                        <span className="flex items-center gap-1"><Clock size={10} /> {new Date(ann.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                        <span>👤 {ROLE_LABELS[ann.targetRole]}</span>
                        {ann.expiresAt && <span>📅 Expires {new Date(ann.expiresAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>}
                      ...
                      {ann.requiresSubmission && (
                        <div className="mt-2 grid grid-cols-3 gap-2 text-xs">
                          {(['passed', 'missed', 'pending'] as const).map(st => {
                            ...
                            return (
                              <div key={st} className="rounded-lg bg-gray-50 border border-gray-200 p-2">
                                <p className="font-semibold text-gray-700 capitalize">{st}</p>
                                <p className="text-gray-500">{list.length} employee(s)</p>
                              </div>
                            );
                          })}
                        </div>
          ...
          >
            ...
            >
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  <label className="text-xs font-semibold text-gray-600 block mb-1.5">Comments before publish (optional)</label>
                  <textarea
                    value={form.comments}
                ...
                <div>
                  <label className="text-xs font-semibold text-gray-600 block mb-1.5">Submission Deadline (optional)</label>
                  <input
                    type="datetime-local"
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                  ...
                  >
                    <div className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${form.requiresSubmission ? 'translate-x-6' : ''}`} />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                  ...
                  >
                    <div className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${form.isPinned ? 'translate-x-6' : ''}`} />
                  </button>
                </div>
