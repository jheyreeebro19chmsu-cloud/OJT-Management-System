...
import { MapPin, Plus, Edit2, Trash2, CheckCircle, X, Save, ToggleLeft, ToggleRight, Navigation, Info } from 'lucide-react';
...
const BLANK_ZONE = {
  name: '', address: '', lat: 14.5547, lng: 121.0244, radius: 300, active: true,
};
...
export function AdminGeofence() {
  ...
  const handleEdit = (zone: GeofenceZone) => {
    setEditId(zone.id);
    setForm({ name: zone.name, address: zone.address, lat: zone.lat, lng: zone.lng, radius: zone.radius, active: zone.active });
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => { setForm(BLANK_ZONE); setShowAdd(true); }}
      ...
      <div className="bg-blue-50 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-blue-700">
          ...
          <p className="text-blue-600 text-xs leading-relaxed">
            When a trainee attempts to clock in or out, the system checks their GPS coordinates against all active geofence zones. They must be within the specified radius (in meters) from the zone center to proceed with face verification.
          </p>
        </div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}
        ...
        {invalidZones.length > 0 && (
          <div className="mx-4 mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
            <span className="font-semibold">Warning:</span> {invalidZones.length} zone{invalidZones.length > 1 ? 's' : ''} with invalid coordinates {invalidZones.length > 1 ? 'were' : 'was'} skipped on the map.
          </div>
        )}
        <GeofenceMap
          zones={geofenceZones}
          ...
          onPick={(lat, lng) => { upd('lat', lat); upd('lng', lng); }}
        />
      </motion.div>
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${zone.active ? 'bg-blue-100' : 'bg-gray-100'}`}>
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleToggle(zone)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all">
                        {zone.active ? <ToggleRight size={16} className="text-blue-500" /> : <ToggleLeft size={16} />}
                      </button>
                      <button onClick={() => handleEdit(zone)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all">
                        <Edit2 size={14} />
                      </button>
                      <button onClick={() => handleDelete(zone.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <input value={form.name} onChange={e => upd('name', e.target.value)} placeholder="e.g. Main Office"
          className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50" />
      </div>
      <div>
        <label className="text-xs font-semibold text-gray-600 block mb-1">Address</label>
        <input value={form.address} onChange={e => upd('address', e.target.value)} placeholder="Street, City"
          className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50" />
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-xs font-semibold text-gray-600 block mb-1">Latitude *</label>
          <input type="number" step="0.0001" value={form.lat} onChange={e => upd('lat', parseFloat(e.target.value))}
            className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50" />
        </div>
        <div>
          <label className="text-xs font-semibold text-gray-600 block mb-1">Longitude *</label>
          <input type="number" step="0.0001" value={form.lng} onChange={e => upd('lng', parseFloat(e.target.value))}
            className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50" />
        </div>
      </div>
      <div>
        ...
        <input type="range" min="50" max="1000" step="50" value={form.radius} onChange={e => upd('radius', parseInt(e.target.value))}
      ...
      <div className="flex items-center justify-between bg-gray-50 rounded-xl p-3">
        ...
        <button onClick={() => upd('active', !form.active)} className="relative">
          {form.active ? <ToggleRight size={28} className="text-blue-600" /> : <ToggleLeft size={28} className="text-gray-400" />}
        </button>
      </div>
