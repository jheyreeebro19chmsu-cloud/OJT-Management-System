...
export function Announcements() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-xl font-bold text-gray-800">Announcements & Submissions</h2>
        <p className="text-sm text-gray-500">Submit required responses before deadline.</p>
      </div>
        ...
        announcements.map(announcement => {
          ...
          return (
            <div key={announcement.id} className="rounded-2xl bg-white border border-gray-100 p-4 space-y-3">
              ...
              {announcement.photo && (
                <img src={announcement.photo} alt="" className="w-full rounded-xl border border-gray-200 max-h-56 object-cover" />
              )}
              ...
              {announcement.requiresSubmission && (
                <div className="rounded-xl border border-gray-200 p-3 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {photoDrafts[announcement.id] && (
                      <span className="text-[11px] text-green-700">Photo selected</span>
                    )}
                    <button
                      type="button"
                      onClick={() => submit(announcement)}
                      className="ml-auto px-3 py-2 rounded-lg bg-blue-700 text-white text-xs font-medium hover:bg-blue-800"
                    >
                      Send to Host
                    </button>
                  </div>
...
function StatusBadge({ status }: { status: 'passed' | 'missed' | 'pending' }) {
  if (status === 'passed') {
    return <span className="text-xs px-2 py-1 rounded-full bg-green-100 text-green-700 inline-flex items-center gap-1"><CheckCircle size={12} /> Passed</span>;
  }
  if (status === 'missed') {
    return <span className="text-xs px-2 py-1 rounded-full bg-red-100 text-red-700 inline-flex items-center gap-1"><XCircle size={12} /> Missed</span>;
  }
  return <span className="text-xs px-2 py-1 rounded-full bg-amber-100 text-amber-700 inline-flex items-center gap-1"><Bell size={12} /> Pending</span>;
}
