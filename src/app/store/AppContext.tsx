...
type RegisterHostSupervisorInput = Omit<HostSupervisor, 'id' | 'active'> & {
  password?: string;
};
...
interface AppContextType {
  ...
  registerHostSupervisor: (data: RegisterHostSupervisorInput) => HostSupervisor;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const registerHostSupervisor = (data: RegisterHostSupervisorInput): HostSupervisor => {
    const { password, ...hostData } = data;
    if (password) {
      setPasswordForEmail(hostData.email, password);
    }

    const newHost: HostSupervisor = {
      ...hostData,
      id: `host-${Date.now()}`,
      active: true,
    };

    setHostSupervisors(prev => [...prev, newHost]);
    setCurrentUser({ id: newHost.id, name: newHost.name, role: 'host' });
    return newHost;
  };
  ...
  return (
    <AppContext.Provider value={{
      ...
      login, logout, changeCurrentUserPassword, registerEmployee, registerHostSupervisor, updateEmployee, deleteEmployee,
